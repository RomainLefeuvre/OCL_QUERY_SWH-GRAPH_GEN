...
public class SwhGraphProperties implements FlyweightPrototype<SwhGraphProperties> {
    ...
    public SwhGraphProperties copy() {
        return new SwhGraphProperties(path, nodeIdMap.copy(), nodeTypesMap==null ? null :nodeTypesMap.copy(),
                ...
                ((tagNameOffsets instanceof LongMappedBigList)
                        ...
                        : tagNameOffsets),
                this.edgeLabelNames != null?this.edgeLabelNames.copy():null
        );
    }
    ...
    protected static LongBigList loadMappedLongs(String path) throws IOException {
        try (RandomAccessFile raf = new RandomAccessFile(path, "r")) {
            return LongMappedBigList.map(raf.getChannel());
        }
    }

    protected static IntBigList loadMappedInts(String path) throws IOException {
        try (RandomAccessFile raf = new RandomAccessFile(path, "r")) {
            return IntMappedBigList.map(raf.getChannel());
        }
    }

    protected static ShortBigList loadMappedShorts(String path) throws IOException {
        try (RandomAccessFile raf = new RandomAccessFile(path, "r")) {
            return ShortMappedBigList.map(raf.getChannel());
        }
    }

    protected static ByteBigList loadMappedBytes(String path) throws IOException {
        try (RandomAccessFile raf = new RandomAccessFile(path, "r")) {
            return ByteMappedBigList.map(raf.getChannel());
        }
    }

    protected static LongBigList loadEFLongs(String path) throws IOException {
        try {
    ...
    protected static byte[] getLine(ByteBigList byteArray, long start) {
        long end = start;
